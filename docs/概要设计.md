...
| V1.1.0    | 产品研发团队 |        | 2026 年 05 月 09 日 | 根据评审意见修正识别范围、硬件规格、客户端兼容和功能边界 |
...
- 软件系统形态：Web 管理端、桌面客户端、OpenAPI 接口、jusha-asr-business 部署包
...
- 巨鲨语音助手需求规格说明书
- 项目根目录 README
- backend/internal/interfaces/api/openapi_docs.md（嵌入后端二进制的 OpenAPI 对接指南）
- backend/configs/config.example.yaml
- deploy/jusha-asr-business/README.md
...
| 桌面客户端 | Win10/11 使用 Tauri，Win7 使用 Electron 22 | Win10/11 复用系统 WebView2，安装包轻量；Win7 独立产品线使用 Electron 22.3.27 包带 Chromium 108，无需预装 WebView2；两者共用同一套 Vue 代码 (`desktop/src/`)，Win7 包位于 `desktop-electron/`，通过 Vite alias 的 tauri shim 转接 IPC |
...
| 分类         | 名称                     | 说明 |
| ------------ | ------------------------ | ---- |
| 操作系统     | Linux                    | jusha-asr-business 发布和容器部署以 Linux 环境为主 |
...
| 版本 | 最低配置 | 推荐配置 |
| ---- | -------- | -------- |
| 标准版 | CPU 8核 / 内存16G / 存储200G SSD / RTX 3090同等算力 | CPU 16核 / 内存32G / 存储500G SSD / A10或A100 |
| 高级版 | CPU 16核 / 内存32G / 存储500G SSD / A10 | CPU 16核及以上 / 内存32G及以上 / 存储500G SSD及以上 / A100 |
...
| runtime/mysql | jusha-asr-business 下 MySQL 数据目录 |
| runtime/certs | jusha-asr-business 下 HTTPS 证书目录 |
| runtime/tmp | 运行期临时目录 |

说明：当前服务端硬件规格主要取决于外部 ASR 服务和可选说话人服务的部署方案。本仓库内的业务服务层以协调、编排和持久化为主，音频识别算力不在本仓库内部实现。桌面端 Windows 7 仅作为兼容交付目标，采用 `desktop-electron/` 下独立的 Electron 22 工程打包，不再依赖 WebView2 固定版运行时；Win10/11 仍由 Tauri 产出主推荐安装包。两个包在公共下载页根据文件名中的 `_win10_` / `_win7_` 标识分组展示。
...
4. 交付方式完整。jusha-asr-business 发布包内置数据库、服务进程、Nginx 和下载页，便于内网交付。
...
7. **jusha-asr-business 一体化交付**：发布包内置 MySQL、Nginx 和三类业务服务，同时保留对外部识别服务的连接能力。
...
- 不包含角色矩阵配置页面，细粒度权限由后端接口权限与后续权限模型控制。
...
| 系统运营子模块 | 由登录、用户管理、OpenAPI 管理、公开下载组成 | 登录为后台页面提供 JWT；OpenAPI 管理配置开放平台应用、能力和默认工作流；公开下载页与下载目录挂载内容联动 |
...
- 中心区域作为拖动热区，短点击与拖动通过位移阈值区分，避免边缘点击误触拖动。
...
- Windows 全局热键由原生热键线程管理，鼠标侧键命中配置后会拦截系统 Back/Forward 事件，避免热键动作外溢到前台应用。
...
6. 仪表板统计聚合接口。
7. 公开下载列表与下载文件对接。
8. OpenAPI 应用管理、能力授权、密钥轮换、调用日志查看和对接文档展示。
...
| 词典运营子模块 | 提供术语、纠错规则、敏感词、语气词、控制指令管理，并支持术语CSV/TSV/TXT/XLSX批量导入 | 为 NLP 处理和工作流节点提供基础数据；纠错规则按优先级、排序和ID稳定执行 |
| 应用设置子模块 | 提供语音控制等全局配置读写 | 被桌面端配置同步和语音控制运行时消费 |
| 运营支撑子模块 | 提供数据看板、下载列表和公开下载能力 | 数据看板调用转写服务聚合统计；下载列表与发布目录挂载内容联动 |
| 开放平台管理子模块 | 提供 OpenAPI 应用、能力、密钥、调用日志和对接文档管理 | 其配置结果由 asr-api 和 nlp-api 在运行期通过 OpenAuth 中间件读取；对接文档支持能力筛选、关键词搜索和复制 |
...
5. 识别语种默认为 `auto`，OpenAI 兼容外部 ASR 调用在 `auto` 或空值时不透传 `language` 字段，以保留中英文和数字混合识别能力；仅调用方显式传入非自动语种时才透传。
6. Web 管理端上传入口当前按产品规格限制为 `.wav` / `.mp3`，URL 提交仍以后端和外部 ASR 能力校验为准。
...
| 表/对象 | 作用 | 关键关系 |
| ------- | ---- | -------- |
| term_dicts / term_entries | 术语词库和词条 | 为术语纠错节点提供基础数据；管理端批量导入最多5000行且跳过重复标准词 |
| correction_rules | 纠错规则表 | 与术语词典关联，按 priority、sort_order、id 稳定排序执行 |
...
6. **发布回滚机制**：jusha-asr-business 安装脚本在升级后会结合健康检查决定是否回滚到上一版本镜像。
...
2. jusha-asr-business 中服务启动顺序为 MySQL、业务服务、Gateway、Nginx。
...
| 外部依赖 | jusha-asr-business 发布包不内置外部 ASR 和可选说话人服务 | 部署时需要额外准备外部服务 | 继续通过环境变量和发布文档约束部署流程 |
