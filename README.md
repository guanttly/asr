...
- 自动构建 Tauri 桌面端时，如果缺少 Rust、Windows MSVC target 或 cargo-xwin，发布脚本会默认尝试自动安装，并默认使用 rsproxy Rust/Cargo 国内镜像；如需禁用自动安装可设置 `ASR_RELEASE_AUTO_INSTALL_RUST=0`，如需禁用镜像可设置 `ASR_RELEASE_USE_RUST_MIRROR=0`
- 自动构建 Win7 兼容版时，Linux 服务器默认使用 Docker Wine + xvfb，无需桌面 UI；如需改用宿主机 Wine，可设置 `ASR_RELEASE_ELECTRON_WIN_BUILD_MODE=host`
