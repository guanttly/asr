...
- 打包时如果传入 `--server-host` 和端口参数，桌面客户端会以内置 HTTPS 默认地址重新构建，并自动放到 `runtime/downloads`。桌面端安装包版本默认读取 `desktop/package.json`，也可传 `--desktop-version <version>` 显式指定；自动构建 Tauri 时缺少 Rust/cargo-xwin 会默认尝试自动安装，可用 `ASR_RELEASE_AUTO_INSTALL_RUST=0` 禁用。
