...
const ISSUE_OVERRIDES: Record<string, Omit<DesktopUseCaseAssessment, 'id' | 'title'>> = {
  121310: {
    ...
    area: '麦克风热插拔',
    marker: 'OK',
    conclusion: '录音器同时监听 MediaStreamTrack 的 ended/mute/unmute 与 mediaDevices devicechange；任一信号判定采集中断即释放管道并提示“麦克风已断开，等待重新接入”，进入 MIC 未检测到状态；恢复时由带退避的自动重连控制器(400/800/1600/3000ms)重建 getUserMedia 采集流，不依赖新的设备事件，避免驱动未就绪或被占用时永久卡死；热插拔决策与重连退避逻辑有单测。真实热插拔仍需桌面手测。',
    sourceRefs: ['desktop/src/composables/useAudioRecorder.ts', 'desktop/src/composables/hotplugRecovery.ts', 'desktop/src/components/MicButton.vue', 'desktop/src/stores/app.ts', 'desktop/src/composables/hotplugRecovery.test.ts'],
  },
  121311: {
    kind: 'unit',
    area: '麦克风热插拔',
    marker: 'OK',
    conclusion: '录音中麦克风断开后保留录音会话与 chunk 回调；设备恢复(devicechange/unmute)或退避自动重试成功时重新 getUserMedia 并恢复采集，重连期间若用户停止会主动中止以避免遗留野采集管道；悬浮球 MIC 未检测到与断开/恢复回调已接入。真实设备热插拔仍需桌面手测。',
    sourceRefs: ['desktop/src/composables/useAudioRecorder.ts', 'desktop/src/composables/hotplugRecovery.ts', 'desktop/src/components/MicButton.vue', 'desktop/src/composables/hotplugRecovery.test.ts'],
  },
  121326: {
    kind: 'unit',
    area: '配置同步失败',
