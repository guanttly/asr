...
ASR_RELEASE_IMAGE=${ASR_RELEASE_IMAGE:-${RELEASE_IMAGE:-jusha-asr-business:latest}}
...
find_cleanup_image() {
  ...
  if [ "$ASR_RELEASE_IMAGE" != "jusha-asr-business:latest" ] && docker image inspect jusha-asr-business:latest >/dev/null 2>&1; then
    printf '%s\n' 'jusha-asr-business:latest'
    return 0
  fi
...
purge_runtime_data() {
  if rm -rf runtime/config runtime/mysql runtime/certs runtime/downloads runtime/tmp runtime/uploads runtime/logs backups 2>/dev/null; then
    return 0
  fi

  CLEANUP_IMAGE=$(find_cleanup_image || true)
  if [ -z "$CLEANUP_IMAGE" ]; then
    echo "清理失败：当前用户无权删除 runtime 数据目录，且本地找不到可用于容器内清理的业务镜像。" >&2
    return 1
  fi
  ...
  docker run --rm \
    ...
    --entrypoint sh \
    "$CLEANUP_IMAGE" \
    -lc 'rm -rf /cleanup-root/runtime/config /cleanup-root/runtime/mysql /cleanup-root/runtime/certs /cleanup-root/runtime/downloads /cleanup-root/runtime/tmp /cleanup-root/runtime/uploads /cleanup-root/runtime/logs /cleanup-root/backups'
}
...
if [ "$REMOVE_IMAGE" -eq 1 ]; then
  ...
  docker image rm jusha-asr-business:latest >/dev/null 2>&1 || true
fi
